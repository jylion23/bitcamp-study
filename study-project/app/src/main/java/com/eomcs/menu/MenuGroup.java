...
public class MenuGroup extends Menu {
  ...
  // 출력될 메뉴 목록 준비
  // => 왜? 
  // - 메뉴 출력 속도를 높이기 빠르게 하기 위함이다
  // - 메뉴를 출력할 때마다 매번 출력할 메뉴와 출력하지 말아야할 메뉴를 구분하는 시간을 줄이기위함
  //
    ...
    for (int i = 0; i < this.size; i++) {
      if (this.childs[i].enableState == Menu.ENABLE_LOGOUT && 
        ...
        menuList.add(this.childs[i]);
          ...
          AuthHandler.getLoginUser() != null) {
        // 로그인이 되어 있을 때만 출력하는 메뉴인데 로그인 되어 있지 않으면 추가하지 않는다.
        menuList.add(this.childs[i]);
  ...
  private Menu selectMenu(List<Menu> menuList) {
    ...
    if (menuNo == 0 && !disablePrevMenu) {
      return prevMenu; // 호출한 쪽에 이전 메뉴를 선택을 알리기 위해
    } 
